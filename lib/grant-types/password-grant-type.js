...
PasswordGrantType.prototype.handle = function(request, client) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getUser(client, request);
    })
    .then(function(user) {
...
PasswordGrantType.prototype.getUser = function(client, request) {
  ...
  return promisify(this.model.getUser, 3).call(this.model, client.clientId, request.body.username, request.body.password)
    .then(function(user) {
      if (!user) {
  ...
  return Promise.all(fns)
    ...
    .spread(function(scope, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
      ...
      if (user.twoFactorEnabled )
      {
        ...
        token.activation_code = getNewCode(user._id);
      }
